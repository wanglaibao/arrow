...
// HCons(head=1000, tail=HCons(head=900, tail=arrow.generic.HNil@50e2e62c))
```

```kotlin
Account(1000, 900).toHListLabeled()
// HCons(head=Tuple2(a=balance, b=1000), tail=HCons(head=Tuple2(a=available, b=900), tail=arrow.generic.HNil@50e2e62c))
```
...
// DeferredK(deferred=LazyDeferredCoroutine{New}@1a1415f6, scope=kotlinx.coroutines.GlobalScope@dcf31f4)
```
